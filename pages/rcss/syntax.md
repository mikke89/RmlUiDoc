...
* In the format `hsl(h, s%, l%)`{:.value} or `hsla(h, s%, l%, a)`{:.value}. `h` is the hue in degrees, and is wrapped into [0, 360) internally. `s` and `l` are saturation and lightness as percentage values from 0 to 100. `a` is the alpha value from 0.0 to 1.0.
