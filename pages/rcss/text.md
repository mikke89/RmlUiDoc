...
`clip`{:.value}
: Overflowing text is clipped like normal around the padding area of the element.

`ellipsis`{:.value}
: For overflowing text, an ellipsis is inserted at the end of the text just before being clipped.

`<string>`{:.value}
: For overflowing text, the specified string is inserted at the end of the text just before being clipped.

_Note_: To clip between characters instead of the padding area, you can specify an empty string in RCSS, i.e. `""`.
