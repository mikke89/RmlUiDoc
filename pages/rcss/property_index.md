...
---- | ------ | ------------- | ---------- | ---------- | ----------- | -----
`animation`{:.prop} | See [animations](animations_transitions_transforms.html#animations) | none | all | no | N/A | 
...
`decorator`{:.prop} | none \| \<name\> \| \<type\>( \<properties\> ) | none | all | no | N/A | See [decorators](decorators.html) for details.
`display`{:.prop} | inline \| block \| inline-block \| none | inline | all | no | N/A | 
`drag`{:.prop} | none \| drag \| drag-drop \| block \| clone | none | all | no | N/A | Introduced for RCSS. Controls generation of drag messages.
`focus`{:.prop} | none \| auto | auto | all | yes | N/A | 
`font`{:.prop} | `font-style`{:.prop} `font-weight`{:.prop} `font-size`{:.prop} `font-family`{:.prop} | | | | | 
`font-effect`{:.prop} | none \| \<type\>( \<properties\> ) | none | all | yes | N/A | See [font effects](font_effects.html) for details.
...
`image-color`{:.prop} | \<color\> | white | \<img\> elements and decorators | no | N/A | 
...
`perspective`{:.prop} | none \| \<length\> | none | all | no | N/A | See [transforms](animations_transitions_transforms.html#transform-property).
`pointer-events`{:.prop} | auto \| none | auto | all | yes | N/A | 
...
`text-decoration`{:.prop} | none \| underline \| overline \| line-through | none | all | yes | N/A | 
`text-transform`{:.prop} | none \| capitalize \| uppercase \| lowercase | none | all | yes | N/A | 
`top`{:.prop} | \<length\> \| \<percentage\> | 0px | positioned elements | no | height of containing block | No 'auto'.
`transition`{:.prop} | See [transitions](animations_transitions_transforms.html#transitions) | none | all | no | N/A | 
`transform`{:.prop} | See [transforms](animations_transitions_transforms.html#transform-property) | none | all | no | N/A | 
...
`white-space`{:.prop} | normal \| pre \| nowrap \| pre-wrap \| pre-line | normal | block-level elements | yes | N/A | 
`width`{:.prop} | \<length\> \| \<percentage\> \| auto | auto | block and replaced inline elements | no | width of containing block | 
`z-index`{:.prop} | \<number\> \| auto | auto | all | no | N/A | Applies to all elements. For documents, 'auto' allows pulling to front, otherwise remains at top or bottom. 